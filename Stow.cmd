...
Stow:
	var Stow.command $0
Stowing:
	gosub Send Q "stow %Stow.command" "^You pick up|^You put your" "^But that's closed\.$|^I can't find your container for stowing things in\!  Type STORE HELP for information on how to set up your containers\.$|^You just can't get the .+ to fit|^^That's too heavy to go in there\!$"
	...
	if (matchre("%Send.message", "^You just can't get the \S+ (\bbox\b|\bcaddy\b|\bcasket\b|\bchest\b\b|\bcoffer\b|\bcrate\b|\bskippet\b|\bstrongbox\b|\btrunk\b) to fit in .+, no matter how you arrange it\.$")) then {
		gosub Error %Send.message Dropping box instead.
		gosub drop my $0
	}
